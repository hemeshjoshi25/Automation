...
class RunTripTypeProfile(IscE2eTestCase):
    """Run a Trip Type Profile Report"""

    def setUp(self):
        """Set Up"""
        super(RunTripTypeProfile, self).setUp()
    ...
    def test_b_category(self):
        self.select.search_hierarchy_select(Prompts.CATEGORY.value, TTP.CAT.value, TTP.CAT_LEV.value, TTP.CAT_SEL.value)

    def test_c_product_level(self):
        self.select.search_input(Prompts.PRODUCT_LEVEL.value, TTP.PROD_LEV.value)

    def test_d_store(self):
        self.select.search_hierarchy(Prompts.STORE.value, TTP.STORE.value, TTP.STORE_LEV.value)
