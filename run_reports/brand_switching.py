...
class RunBrandSwitching(IscE2eTestCase):
    """Run a Brand Switching Report"""

    def setUp(self):
        """Set Up"""
        super(RunBrandSwitching, self).setUp()
        # Initialze Page Objects
    ...
    def test_b_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, BS.PROD.value, BS.PROD_LEV.value)

    def test_c_category(self):
        self.select.search_hierarchy_select(Prompts.CATEGORY.value, BS.CAT.value, BS.CAT_LEV.value, BS.CAT_SEL.value)
