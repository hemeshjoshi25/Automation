...
class RunLapsedRepeatNew(IscE2eTestCase):
    """Run a Lapsed, Repeat, New Report"""

    def setUp(self):
        """Set Up"""
        super(RunLapsedRepeatNew, self).setUp()
    ...
    def test_b_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, LRN.PROD.value, LRN.PROD_LEV.value)

    def test_c_category(self):
        self.select.search_hierarchy_select(Prompts.CATEGORY.value, LRN.CAT.value, LRN.CAT_LEV.value, LRN.CAT_SEL.value)
