...
class RunEventAnalysis(IscE2eTestCase):
    """Run an Event Analysis Report"""

    def setUp(self):
        """Set Up"""
        super(RunEventAnalysis, self).setUp()
    ...
    def test_b_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, EA.PROD.value, EA.PROD_LEV.value)

    def test_c_category(self):
        self.select.search_hierarchy_select(Prompts.CATEGORY.value, EA.CAT.value, EA.CAT_LEV.value, EA.CAT_SEL.value)
    ...
    def test_i_adv_post_period(self):
        ...
        self.select.basic_input(Prompts.NUM_WEEKS.value, EA.POST.value)
