...
class RunBuyerLoyaltyFlow(IscE2eTestCase):
    """Run a Buyer Loyalty Flow Report"""

    def setUp(self):
        """Set Up"""
        super(RunBuyerLoyaltyFlow, self).setUp()
    ...
    def test_b_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, BLF.PROD.value, BLF.PROD_LEV.value)

    def test_c_category(self):
        self.select.search_hierarchy_select(Prompts.CATEGORY.value, BLF.CAT.value, BLF.CAT_LEV.value, BLF.CAT_SEL.value)
