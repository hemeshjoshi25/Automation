...
class RunHouseholdAffinity(IscE2eTestCase):
    """Run a Household Affinity Report"""

    def setUp(self):
        """Set Up"""
        super(RunHouseholdAffinity, self).setUp()
    ...
    def test_b_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, HA.PROD.value, HA.PROD_LEV.value)

    def test_c_category(self):
        self.select.search_hierarchy_select(Prompts.CATEGORY.value, HA.CAT.value, HA.CAT_LEV.value, HA.CAT_SEL.value)
