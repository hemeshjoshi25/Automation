...
class RunPromotionEffectiveness(IscE2eTestCase):
    """Run a Promotion Effectiveness Report"""

    def setUp(self):
        """Set Up"""
        super(RunPromotionEffectiveness, self).setUp()
    ...
    def test_b_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, PE.PROD.value, PE.PROD_LEV.value)

    def test_c_category(self):
        self.select.search_hierarchy_select(Prompts.CATEGORY.value, PE.CAT.value, PE.CAT_LEV.value, PE.CAT_SEL.value)
