...
class RunShopperMetrics(IscE2eTestCase):
    """Run a Shopper Metrics Report"""

    def setUp(self):
        """Set Up"""
        super(RunShopperMetrics, self).setUp()
    ...
    def test_b_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, SM.PROD.value, SM.PROD_LEV.value)

    def test_c_category(self):
        self.select.search_hierarchy_select(Prompts.CATEGORY.value, SM.CAT.value, SM.CAT_LEV.value, SM.CAT_SEL.value)
