...
class RunCrossPurchase(IscE2eTestCase):
    """Run a Cross Purchase Report"""

    def setUp(self):
        """Set Up"""
        super(RunCrossPurchase, self).setUp()
    ...
    def test_b_product(self):
        self.select.search_hierarchy_select(Prompts.PRODUCT.value, CP.PROD.value, CP.PROD_LEV.value, CP.PROD_SEL.value)
    ...
    def test_d_comp_product(self):
        self.select.search_hierarchy_select(Prompts.COMP_PRODUCT.value, CP.COMP_PROD.value,
                                            CP.COMP_PROD_LEV.value, CP.COMP_PROD_SEL.value)
    ...
    def test_h_adv_focus_cat(self):
        self.select.search_hierarchy_select(Prompts.CATEGORY.value, CP.FOCUS_CAT.value,
                                            CP.FOCUS_CAT_LEV.value, CP.FOCUS_CAT_SEL.value)

    def test_i_adv_focus_store(self):
        self.select.search_hierarchy(Prompts.STORE.value, CP.FOCUS_STORE.value, CP.FOCUS_STORE_LEV.value)
