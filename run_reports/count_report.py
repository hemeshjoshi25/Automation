...
class RunCountReport(IscE2eTestCase):
    """Run a Count Report"""

    def setUp(self):
        """Set Up"""
        super(RunCountReport, self).setUp()
    ...
    def test_b_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, CR.PROD.value, CR.PROD_LEV.value)

    def test_c_category(self):
        self.select.search_hierarchy_select(Prompts.CATEGORY.value, CR.CAT.value, CR.CAT_LEV.value, CR.CAT_SEL.value)
