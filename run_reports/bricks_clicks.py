...
class RunBricksClicks(IscE2eTestCase):
    """Run a Bricks and Clicks Report"""

    def setUp(self):
        """Set Up"""
        super(RunBricksClicks, self).setUp()
    ...
    def test_b_category(self):
        self.select.search_hierarchy_select(Prompts.CATEGORY.value, BC.CAT.value, BC.CAT_LEV.value, BC.CAT_SEL.value)

    def test_c_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, BC.PROD.value, BC.PROD_LEV.value)
