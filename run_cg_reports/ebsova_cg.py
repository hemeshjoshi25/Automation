...
class RunEBSOVACG(IscE2eTestCase):
    """Run an EBSOVA Report with Custom Groups"""

    def setUp(self):
        """Set Up"""
        super(RunEBSOVACG, self).setUp()
    ...
    def test_b_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, EBSOVA.CG_PROD.value, EBSOVA.CG_PROD_LEV.value)

    def test_c_category(self):
        self.select.search_hierarchy(Prompts.CATEGORY.value, EBSOVA.CG_CAT.value, EBSOVA.CG_CAT_LEV.value)
