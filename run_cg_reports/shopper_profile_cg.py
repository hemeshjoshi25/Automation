...
class RunShopperProfileCG(IscE2eTestCase):
    """Run a Shopper Profile Report with Custom Groups"""

    def setUp(self):
        """Set Up"""
        super(RunShopperProfileCG, self).setUp()
    ...
    def test_e_adv_product(self):
        ...
        self.select.search_hierarchy(Prompts.PRODUCT.value, SP.CG_PROD.value, SP.CG_PROD_LEV.value)

    def test_f_adv_category(self):
        self.select.search_hierarchy(Prompts.CATEGORY.value, SP.CG_CAT.value, SP.CG_CAT_LEV.value, 0)
    ...
    def test_h_adv_comp_product(self):
        self.select.search_hierarchy(Prompts.COMP_PRODUCT.value, SP.CG_COMP_PROD.value, SP.CG_COMP_PROD_LEV.value)

    def test_i_adv_comp_category(self):
        self.select.search_hierarchy(Prompts.CATEGORY.value, SP.CG_COMP_CAT.value, SP.CG_COMP_CAT_LEV.value)
