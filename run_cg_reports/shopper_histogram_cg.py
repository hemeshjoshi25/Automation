...
class RunShopperHistogramCG(IscE2eTestCase):
    """Run a Shopper Histogram Report with Custom Groups"""

    def setUp(self):
        """Set Up"""
        super(RunShopperHistogramCG, self).setUp()
    ...
    def test_b_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, SH.CG_PROD.value, SH.CG_PROD_LEV.value)

    def test_c_category(self):
        self.select.search_hierarchy(Prompts.CATEGORY.value, SH.CG_CAT.value, SH.CG_CAT_LEV.value)
