...
class RunBrandDiagnosticsCG(IscE2eTestCase):
    """Run a Brand Diagnostics Report with Custom Groups"""

    def setUp(self):
        """Set Up"""
        super(RunBrandDiagnosticsCG, self).setUp()
    ...
    def test_b_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, BD.CG_PROD.value, BD.CG_PROD_LEV.value)

    def test_c_category(self):
        self.select.search_hierarchy(Prompts.CATEGORY.value, BD.CG_CAT.value, BD.CG_CAT_LEV.value)
