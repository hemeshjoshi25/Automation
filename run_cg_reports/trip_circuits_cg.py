...
class RunTripCircuitsCG(IscE2eTestCase):
    """Run a Trip Circuits Report with Custom Groups"""

    def setUp(self):
        """Set Up"""
        super(RunTripCircuitsCG, self).setUp()
    ...
    def test_c_product(self):
        self.select.search_hierarchy(Prompts.PRODUCT.value, TC.CG_PROD.value, TC.CG_PROD_LEV.value)

    def test_d_category(self):
        self.select.search_hierarchy(Prompts.CATEGORY.value, TC.CG_CAT.value, TC.CG_CAT_LEV.value)
    ...
    def test_i_adv_group_prod_lev(self):
        self.select.search_input(Prompts.STORE_LEVEL.value, TC.STORE_LEVEL.value)
